...
概要设计（High-Level Design）处理的是系统级决策，而不是实现细节。它关心的是模块职责、部署拓扑、调用关系和关键技术选型。
...
概要设计的目标，是把“系统做什么”进一步收束为“系统怎么分层、怎么分责、怎么交互”。它回答的是结构问题，而不是代码问题。

### 3. 详细设计：把模块级行为、接口与时序说清楚

详细设计（Low-Level Design）开始进入模块和接口层面。它不是重复概要设计，而是把概要设计中已经确定的结构进一步展开成可实现的局部设计。
...
如果说概要设计解决的是“模块如何组织”，那么详细设计解决的就是“模块内部如何运作，以及模块之间如何精确交互”。
...
测试验证的输出物不是“测试过了”，而是可复现、可度量、可归档的验证结论。
...
对模型而言，模糊形容词几乎没有约束力，真正有效的是明确的规格边界。

### 2. 概要设计：仍然是最核心的人类决策层

AI 可以帮助生成架构图、整理方案对比、列出选型项，但概要设计本身依然高度依赖人类判断。原因在于，系统级 trade-off 通常绑定着团队能力、历史包袱、业务阶段和未来演进方向，这些上下文不可能被一次代码生成自动推导完整。
...
过去大量详细设计文档是为了让实现人员充分理解设计意图；现在其中一部分内容可以被模型自动展开，因此更有价值的详细设计，开始聚焦于那些最容易被误生成的部分：
...
换句话说，详细设计正从“尽量写全”转向“优先写清最不能错的部分”。
