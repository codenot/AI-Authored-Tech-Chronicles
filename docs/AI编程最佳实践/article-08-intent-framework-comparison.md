# 从模糊意图到可执行计划：五大 Agent 开发框架的意图理解与需求拆分机制对比

> **系列背景**：本文是《AI 编程最佳实践》第八篇。前七篇依次建立了从 AI 辅助开发概论、工具链选型、认知约束、团队工作流、知识复利（compound-engineering）到自改进 Agent 的完整工程认知体系。本篇视角横向展开：面对同一个核心问题——「如何阻止 Agent 在意图不清的情况下直接写代码」——五个影响力显著的开源框架给出了截然不同的工程答案。分析基于各框架真实 SKILL.md / AGENTS.md 源码，不做理论推演。
...
| **compound-engineering** | 工程知识应随每次工作迭代而自我积累 | **复利循环**：Brainstorm → Plan → Work → Review → Compound |
...
> 信息来源：[`EveryInc/compound-engineering-plugin`](https://github.com/EveryInc/compound-engineering-plugin) — `plugins/compound-engineering/skills/ce-brainstorm/SKILL.md` + `ce-plan/SKILL.md`，读取于 2026-04-09
...
这不是在说「如何把一个需求拆清榀」，而是在提一个更长的时间轴问题：**每次工作循环是否在积累可复用的工程知识**。其他四个框架在意图层和规划层设置门控。compound-engineering 在此之上加转了一个违展循环：**Compound**——将解决过的问题文档化为可复用知识，下次执行相似任务时由 `learnings-researcher` 自动検索。

### 意图理解门控：`ce:brainstorm` 的多层自适应

[`ce:brainstorm`](https://raw.githubusercontent.com/EveryInc/compound-engineering-plugin/main/plugins/compound-engineering/skills/ce-brainstorm/SKILL.md) 最有意思的设计是其层层自适应的内嵌编排：
...
这个设计反转了其他框架的逆辑辑：**可以短路的**。框架不强制每个任务都经历全流程，而是根据需求清晰度自动加速：

- **Lightweight**：直接确认，跳过 Pressure Test
- **Standard**：完整走 Dialogue + Pressure Test + 生成需求文档
- **Deep**：额外 6-12 月维度的战略性评估

### 核心源码：Product Pressure Test 分级按压

compound-engineering 的 Pressure Test 分三个级别对应不同规模，所出题场景层次清晰：
...
与 gstack 的 YC Partner 风格局有相似，但关键差异在于：gstack 是全时段对抗讯问，compound-engineering 的对抗强度随工作规模动态调整——轻量任务不会浪费标准閮问。

### 核心源码：ce:plan 的「源文件作为资寻默认路径」
...
`ce:plan` 在启动时会自动在 `docs/brainstorms/` 中搜索最新的 `*-requirements.md`，实现意图层⇒规划层的自动衔接。如果找不到匹配的需求文档，它会确认是否先返回 `ce:brainstorm`。

### Compound 循环：和其他框架的根本性差异

其他公四个框架的流程在 Review 后结束。compound-engineering 的循环是：
...
`/ce:compound` 让 Agent 在完成工作后主动将控制局透赵文档化为结构化小知识库。下次 `ce:plan` 启动时，`learnings-researcher` 会并行检索所有相关知识入规划上下文。这就是「复利效应」在工程流程中的具少化体现：工作次数越多， Agent 对这个仓库了解就越深。

### 内置 Document Review 屏障

待需求文档写入后，`ce:brainstorm` 自动调用 `document-review` skill，由 7 个专辟审查工具 Agent（`coherence-reviewer`、`feasibility-reviewer`、`scope-guardian-reviewer` 等）进行周妻审查，并将对课题的任何残留问题带进下一阶段：
...
compound-engineering 的复利设计前提是同一仓库的长期跟踪。对于少于 5 次工作循环的短期项目，`docs/solutions/` 的价値无法体现。此外，当前版本（v2.63）对 Claude Code 有新安装依赖，跨平台转换（Codex、Gemini CLI 等）逻辑由 CLI 中转工具处理，运行时平台差异有一定节调成本。
...
| **切分策略** | 垂直切片（功能路径优先）| TDD 驱动（先测试后实现）| 并行评审链 | PRD + Test Spec 双轨配套 | 半自动类型分汁（Lightw./Standard/Deep） |
| **代码/测试要求** | 无 | 每步必须含真实代码片段 | 无 | test-spec 必须独立成文件 | 每个实现单元必须含测试场景列表 |
| **多 Agent 协作** | ❌ | 有（子 Agent 执行） | ✅（CEO + Eng + Design 评审）| ✅（Planner + Architect + Critic）| ✅（50+ 专辟 Review Agent）|
| **必须 Review 节点** | 无 | 无 | `/plan-eng-review`（强制） | Critic（强制，且顺序执行）| Document Review（强制自动） |
...
| **gstack** | `~/.gstack/…/design-*.md` | `plan.md` + Review 台账表 | ✅ learnings + timeline | ✅ learnings 累积 |
...
- compound-engineering：文档是**知识资产**（`ce:compound` 积累到 `docs/solutions/`，由 `learnings-researcher` 检索备用）

---

## 十、四代「执行门控」的演化路径

回看这五个框架，它们代表了「从意图到执行」门控机制的四个演化阶段：
...
| 长期跦踪、知识积累的团队项目 | **compound-engineering** | Compound 循环 + 自动检索将知识积累转化为内生优势 |
